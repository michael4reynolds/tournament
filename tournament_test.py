...
def testReportMatches():
    ...
    standings = playerStandings()
    if len(standings) != 4:
        raise ValueError("Match deletion should not change number of players in"
                         " standings.")
    for (i, n, w, m) in standings:
        if m != 0:
...
def test_tie_games():
    """
    Test that player ratings are calculated properly after tied games
    """
    deleteMatches()
    deletePlayers()
    registerPlayer("Booker T")
    registerPlayer("Frederick D")
    registerPlayer("Harriet T")
    registerPlayer("Sojourner T")
    standings = playerStandings()
    [id1, id2, id3, id4] = [row[0] for row in standings]
...
def test_block_rematches():
    """
    Test rematches are blocked from submission
    """
    matches = []
    player1 = (1, "Michael")
    player2 = (1, "Angelo")

    add_match(player1, player2, matches)
    try:
        # add same match
        add_match(player1, player2, matches)
    except ValueError:
        print('12. Adding identical match throws')

    try:
        # Add inverse of same match
        add_match(player2, player1, matches)
    except ValueError:
        print('13. Adding inverse of added match throws')
